...
totp_secret = os.getenv("FYERS_2FA")  # Secret key for TOTP generation
fyers_pin = os.getenv("FYERS_PIN")  # Secret key for TOTP generation
...
totp_key = pyotp.TOTP(totp_secret).now()  # totp_key (ex., "OMKRABCDCDVDFGECLWXK6OVB7T4DTKU5")
username = username  # Fyers Client ID (ex., "TK01248")
pin = os.getenv("FYERS_PIN")  # four-digit PIN
client_id = client_id  # App ID of the created app (ex., "L9NY305RTW-100")
secret_key = secret_key  # Secret ID of the created app
redirect_uri = redirect_uri  # Redircet URL you entered while creating the app (ex., "https://trade.fyers.in/api-login/redirect-uri/index.html")
